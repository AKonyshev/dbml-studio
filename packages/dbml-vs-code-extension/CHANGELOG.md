...
- **Edit a column, or rename a table, from the diagram.** Changing a schema used to mean leaving the diagram, finding the table in the `.dbml`, finding the line, editing it and coming back. Point at a column and press `F2`, or double-click it, and a small box opens on the column holding the line exactly as it is in the file. `Enter` applies, `Escape` throws the edit away, `Shift+Enter` adds a line break inside a note, and clicking anywhere else applies what you typed, the way a spreadsheet cell does. `Ctrl+Enter` adds a column below and puts the box on the new one, `Ctrl+Delete` removes one, `Ctrl+Up` / `Ctrl+Down` move one, and `Tab` walks to the row drawn below. You do not have to open the box to add a column: point at one and press `Ctrl/Cmd+Enter`, which is **DBML: Add a column below the one you are pointing at** in the palette. Pointing at a table rather than at one of its columns adds the column at the end of it. `Ctrl/Cmd+Delete` takes one out, aiming at the outlined column first and at the pointer only when nothing is outlined — the opposite of `F2`, because opening the wrong box costs an `Escape` and deleting the wrong column costs the column. Because a MacBook's delete key sends backspace, `Cmd+Backspace` does it too. A refusal — a relation still pointing at the column, or a table left with none — is said out loud rather than leaving the key looking broken. An added column is called `new_column`, or `new_column_2` where the table already holds that name, because three columns of one name is not a schema any database would take. Adding one to a table that is not showing every column puts it in the file and says so, rather than looking like a key that did nothing. A column is aimed at by where it stands rather than by what it is called, so a table that does declare two of one name can still be edited a row at a time and the outline sits on the row you are pointing at. With the pointer on a table but not on one of its columns, `F2` renames the table, and the rename follows through to every relation that names it and to the layout the file carries — including the shapes a real schema actually has: a table whose whole qualified name is one quoted string (`Table "sch.orders"`), a table that lives in a schema of its own, and relations written as `"sch"."a"."id" < "sch"."b"."a_id"`. A renamed table stays where you had dragged it. The command is **DBML: Edit the column, or rename the table** in the palette and can be rebound like every other. Only the ranges you actually changed are written, so one undo takes back one edit; the diagram redraws as soon as the edit lands, while the file is still unsaved; and text that would not parse is refused before it reaches the file rather than after. A file carrying an index that names a column nobody declared — which the diagram has always drawn without complaint — no longer refuses every edit in it with an error about a table you never touched. The diagram on the website is unchanged: editing is something the extension lends it, and the website lends nothing.
