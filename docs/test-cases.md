...
| 10a.1a  | The same column name in two tables        | Move the pointer from one table's `id` onto another table's `id`         | The second one holds the pointer; `F2` opens it, not its table                    | `hoverStore.test.ts`         |
| 10a.1b  | Two columns of one name in a table        | Point at the second of two columns called `note`                         | Only that row is outlined; the first keeps none                                   | `columnFocus.test.ts`        |
| 10a.1c  | Point at a table in pan mode              | Click a table without leaving pan mode                                   | It is outlined, and a column that was pointed at lets go                          | `tableClickIntent.test.ts`   |
| 10a.1d  | Click away from a table                   | Click empty canvas in pan mode                                           | The outline goes; in select mode the marquee still owns that click                | Manual                       |
| 10a.1e  | Shift belongs to select mode              | Hold Shift and click a second table in pan mode                          | One table stays outlined; only select mode builds a group                         | `tableClickIntent.test.ts`   |
...
| 10a.9e  | A column deleted in a Windows file        | Delete a column in a file whose lines end `CRLF`                         | The whole line goes, both halves of its ending with it; no blank line is left     | `columnOps.test.ts`          |
| 10a.9f  | A column added to a Windows file          | Add a column in a file whose lines end `CRLF`                            | The new line ends the way the file already does; the endings stay uniform         | `columnOps.test.ts`          |
| 10a.9g  | A key pressed twice while the host thinks | Press `Enter`, then press it again before anything happens               | One change is sent, not two; `Ctrl+Enter` twice adds one column                   | `QuickEditPopup.test.tsx`    |
| 10a.9h  | Two columns of one name                   | Edit the second of a table's two columns called `note`                   | That line changes, and the first of the two is left alone                         | `columnOps.test.ts`          |
| 10a.9i  | Add a column with no box open             | Point at a column and press `Ctrl/Cmd+Enter`                             | A column is added below it and the box opens on the new one                       | `matchShortcut.test.ts`      |
| 10a.9j  | Add three columns in a row                | Press `Ctrl/Cmd+Enter` three times over                                  | They are called `new_column`, `new_column_2`, `new_column_3`                      | `schemaIndexStore.test.ts`   |
| 10a.9k  | The box lands on the column just added    | Add a column from inside the box                                         | The box is on the new row, not the one it was on                                  | `QuickEditPopup.test.tsx`    |
| 10a.9l  | Add a column to a folded table            | Press `Ctrl/Cmd+Enter` with the table showing key columns only           | The column reaches the file and VS Code says where to look for it                 | `addColumn.test.ts`          |
| 10a.9m  | Add a column to a table, not below one    | Point at a table with no column pointed at, press `Ctrl/Cmd+Enter`       | A column is added at the end of it and the box opens on it                        | `addColumn.test.ts`          |
| 10a.9n  | Add one to a table with no columns        | The same, on a table declaring none                                      | Nothing is written; VS Code says to write the first one in the text               | `addColumn.test.ts`          |
| 10a.9o  | Delete a column with no box open          | Point at a column and press `Ctrl/Cmd+Delete`                            | The column goes; `Cmd+Backspace` does the same on a MacBook                       | `matchShortcut.test.ts`      |
| 10a.9p  | Delete aimed at a table                   | Point at a table rather than a column and press it                       | Nothing is written and VS Code says to point at a column                          | Manual                       |
| 10a.9q  | Delete refused out loud                   | Delete a column another table's `Ref` points at, with no box open        | Nothing is written and the parser's reason reaches the reader                     | Manual                       |
| 10a.9r  | Delete aims at the outline, not the mouse | Click one column, hover another, press `Ctrl/Cmd+Delete`                 | The outlined column goes; with none outlined the hovered one does                 | `quickEditTarget.test.ts`    |
...
| 10a.13h | A column opened after a rename            | Rename a table, then double-click one of its columns                     | The box holds the column's line, not the name just typed into it                  | `QuickEditPopup.test.tsx`    |
...
| 10a.19a | The document goes away mid-edit           | Apply an edit, then close or replace the schema before the reply lands   | The box answers that the document can no longer be edited; it does not hang       | Manual                       |
| 10a.19b | A file unlocked while the diagram is open | Take the read-only bit off a file the diagram called uneditable          | The next redraw offers editing again, without reopening the diagram               | `diagramView.test.ts`        |
...
| 10a.21b | A layout entry the block had to escape    | Rename a table whose name carries a backslash                            | The layout entry follows it, so the table keeps its place                         | `renameOp.test.ts`           |
