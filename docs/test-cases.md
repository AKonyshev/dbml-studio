...
| 12.2 | Every suite runs                        | `yarn test`                                          | Eight packages pass, and a package with tests and no `test` script is named as an error | `scripts/test.js`                                               |
...
| 12.7 | The MkDocs plugin packages              | `yarn build:web && yarn workspace mkdocs-dbml build` | A wheel carrying `_vendor/frame/embed.html` and no editor chunk                         | Manual                                                          |

---

## 13. MkDocs plugin

`packages/mkdocs-dbml`. Unlike section 9, nothing here lives in another
repository: the frame it vendors and the plugin that lays it out are both in
this one.

| #     | Case                                                             | Steps                                                                                                          | Expected                                                                                                    | Automated                        |
| ----- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------- |
| 13.1  | The frame and the host script are in the site                    | Build a site with one `dbml` block                                                                             | `_dbml/embed.html`, its assets, `frame-host.js` and `frame-host.css` are in the site; `validate.mjs` is not | `test_build.py`                  |
| 13.2  | A root page points at the frame and its model                    | Build a site with a block on the home page                                                                     | The iframe's `src` names the frame and the model with the query string a root page needs                    | `test_build.py`                  |
| 13.3  | A nested page climbs to the frame                                | Build a site with a block two folders deep                                                                     | The relative URL climbs the right number of levels                                                          | `test_build.py`                  |
| 13.4  | Without `use_directory_urls` the climb is one shorter            | Build the same site with `use_directory_urls: false`                                                           | The relative URL is one segment shorter                                                                     | `test_build.py`                  |
| 13.5  | A model outside `docs_dir` is copied into the site               | Build a site with a block naming a model reached through `..`                                                  | The model lands at `_dbml/models/<path from mkdocs.yml>`                                                    | `test_build.py`                  |
| 13.6  | Only the block that named a theme is marked fixed                | Build a page with one block carrying `theme:` and one without                                                  | Only the first carries `data-dbml-theme-fixed`                                                              | `test_build.py`                  |
| 13.7  | A broken block shows its error and warns                         | Build a page whose block misspells a key                                                                       | The page shows the error; the build log warns, naming the page and the block                                | `test_build.py`                  |
| 13.8  | A finding is a warning that names the block                      | Build a page whose block asks for a table the model does not have                                              | The log warns, naming the page and the block                                                                | `test_build.py`                  |
| 13.9  | `--strict` turns a finding into a failed build                   | `mkdocs build --strict` on the page from 13.8                                                                  | The build fails                                                                                             | `test_build.py`                  |
| 13.10 | Without `node` there is one warning and the pages are whole      | Build with `node` off `PATH`                                                                                   | One warning that models were not checked; every page still has its diagram                                  | `test_build.py`                  |
| 13.11 | A second build carries nothing of the first                      | Build the same config twice                                                                                    | The second build's warnings and script tags are not doubled                                                 | `test_build.py`                  |
| 13.12 | A `docs_dir` file where the frame goes stops the build           | Put a file at `docs/_dbml/embed.html`, build                                                                   | The build fails, naming the clash                                                                           | `test_build.py`                  |
| 13.13 | Plugin config turns off validation and fixes every frame's theme | Set `validate: false` and `theme: dark` in `mkdocs.yml`, build                                                 | No validator warnings; every diagram is fixed to dark                                                       | `test_build.py`                  |
| 13.14 | A failing validator is one warning and the pages are whole       | Point the plugin at a validator script that always exits non-zero, build                                       | One warning that models were not checked; every page still has its diagram                                  | `test_build.py`                  |
| 13.15 | An unreadable model is refused without the build machine's path  | Remove read permission from a model, build                                                                     | The page and the log name the model, never the build machine's absolute path                                | `test_build.py`                  |
| 13.16 | Vendoring takes the frame's graph and none of the editor         | Run `scripts/vendor.mjs` against a built `packages/web/dist`                                                   | The frame's chunks are copied; Monaco and the editor worker are not                                         | `test_vendor.py`                 |
| 13.17 | A manifest file missing from `dist` fails by name                | Delete a file the manifest names, run `vendor.mjs`                                                             | The script fails and names that file                                                                        | `test_vendor.py`                 |
| 13.18 | No manifest says to build the site first                         | Delete `.vite/manifest.json`, run `vendor.mjs`                                                                 | The script fails and names `yarn build:web`                                                                 | `test_vendor.py`                 |
| 13.19 | A second vendor run leaves nothing of the first                  | Run `vendor.mjs` twice into the same output, with a stray file added between runs                              | The stray file is gone after the second run                                                                 | `test_vendor.py`                 |
| 13.20 | The plugin finds the frame and the host script under one folder  | Vendor, then ask the plugin for the files it ships                                                             | Frame files and the host script both resolve under `_dbml/`; `validate.mjs` does not                        | `test_vendor.py`                 |
| 13.21 | The plugin knows which visualizer build it carries               | Vendor, then read the build id                                                                                 | A build id is present, and the validator script is found                                                    | `test_vendor.py`                 |
| 13.22 | A missing frame is said plainly                                  | Ask the plugin for frame files with nothing vendored                                                           | The error names `yarn workspace mkdocs-dbml vendor`                                                         | `test_vendor.py`                 |
| 13.23 | A model in an `exclude_docs` folder is copied by the plugin      | Name a model under a folder listed in `exclude_docs:`, build                                                   | The model lands at `_dbml/models/docs/…` and the iframe points there                                        | `test_build.py`, `test_paths.py` |
| 13.24 | A model in a dot-folder is copied by the plugin                  | Name a model under `docs/.models/`, which MkDocs excludes by default, build                                    | The model lands at `_dbml/models/docs/.models/…` and the iframe points there                                | `test_build.py`                  |
| 13.25 | A block rendered after its page is done stays code               | A hook renders a `dbml` block in `on_page_context` with its own Markdown, as Material's blog does for excerpts | The block stays code; no warning, no finding and no copied model for it                                     | `test_build.py`                  |
| 13.26 | Plugin config refuses a height a block would refuse              | Set the plugin's `height:` to `0`, `-100`, `true` or `12.5`                                                    | The config fails to load, saying the height must be above zero                                              | `test_build.py`                  |
| 13.27 | A line opening with a code span is not a fence                   | A paragraph starting with a ` ```dbml``` ` code span, then a real block                                        | The real block is still claimed                                                                             | `test_extension.py`              |
| 13.28 | A tilde fence may carry backticks in its info string             | A `~~~` fence whose info string holds a backtick, with a `dbml` example inside                                 | The example inside stays verbatim                                                                           | `test_extension.py`              |
| 13.29 | Site in a subdirectory                                           | `mkdocs build`, serve the output from a nested folder                                                          | A diagram finds both the frame and its model                                                                | Manual                           |
| 13.30 | `use_directory_urls` in both positions                           | Build with it on, then off                                                                                     | Diagrams still find the frame and the model in both                                                         | Manual                           |
| 13.31 | A page with two blocks                                           | Build and open a page with two `dbml` blocks                                                                   | Both draw; expanding one collapses the other                                                                | Manual                           |
| 13.32 | The Material theme toggle                                        | Toggle the site's light/dark switch                                                                            | Diagrams without `theme:` follow, without losing the reader's layout; a block with `theme:` does not move   | Manual                           |
| 13.33 | A model outside `docs_dir` via `..`, and `mkdocs serve`          | Point a block at a model reached with `..`, run `mkdocs serve`, edit the model                                 | The diagram draws, and the page redraws when the model changes                                              | Manual                           |
| 13.34 | A build without `node` on `PATH`                                 | Remove `node` from `PATH`, build the real site                                                                 | One warning; every page still has its diagram                                                               | Manual                           |
