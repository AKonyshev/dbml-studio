...
| 13.23 | Site in a subdirectory                                           | `mkdocs build`, serve the output from a nested folder                             | A diagram finds both the frame and its model                                                                | Manual           |
| 13.24 | `use_directory_urls` in both positions                           | Build with it on, then off                                                        | Diagrams still find the frame and the model in both                                                         | Manual           |
| 13.25 | A page with two blocks                                           | Build and open a page with two `dbml` blocks                                      | Both draw; expanding one collapses the other                                                                | Manual           |
| 13.26 | The Material theme toggle                                        | Toggle the site's light/dark switch                                               | Diagrams without `theme:` follow, without losing the reader's layout; a block with `theme:` does not move   | Manual           |
| 13.27 | A model outside `docs_dir` via `..`, and `mkdocs serve`          | Point a block at a model reached with `..`, run `mkdocs serve`, edit the model    | The diagram draws, and the page redraws when the model changes                                              | Manual           |
| 13.28 | A build without `node` on `PATH`                                 | Remove `node` from `PATH`, build the real site                                    | One warning; every page still has its diagram                                                               | Manual           |
| 13.29 | `mkdocs build --strict` on a page asking for a missing table     | Build `--strict` with a block naming a table the model lacks                      | The build fails, naming the page                                                                            | Manual           |
| 13.30 | A ` ```dbml ` block with no `model:`                             | Build a page about the DBML language, showing plain DBML in a `dbml` fence        | It stays highlighted code                                                                                   | Manual           |
| 13.31 | Material instant navigation, from an expanded diagram            | Expand a diagram, then follow a link or press `n`                                 | The page scrolls again                                                                                      | Manual           |
